...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'tasks' | 'content' | 'posts' | 'escalations'>('overview')
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/posts?campaignId=${id}`)
      const data = await res.json()
      if (data.success) {
        setPosts(data.data)
      }
    } catch {
      // Posts fetch failure is non-critical
    } finally {
...
function PostsTab({
  ...
  if (visiblePosts.length === 0) {
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
        <p className="text-gray-500">No posts yet. Approve content and use the Post button in the Content Library.</p>
        <Link href="/content" className="text-blue-600 hover:underline mt-2 inline-block text-sm">
          Go to Content Library
        </Link>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${postStatusColors[post.status]}`}>
                      {post.status}
                    </span>
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                      post.platform === 'facebook' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'
                    }`}>
                      {platformLabels[post.platform] || post.platform}
                    </span>
                  </div>
                  <p className="text-sm font-medium text-gray-900 truncate">
                    {post.content.headline || 'Untitled content'}
                  </p>
